...
JARS={"jgit-storage-hibernate-core","jgit-storage-hibernate-search","jgit-storage-hibernate-java-analysis","jgit-storage-hibernate-architecture","jgit-storage-hibernate-benchmarks"}
...
def main():
    ...
    for path in sorted(Path('.').glob('jgit-storage-hibernate-*/pom.xml')):
        module=ET.parse(path).getroot(); plugins=module.findall("m:build/m:plugins/m:plugin",NS)
        ids={txt(p,"m:artifactId") for p in plugins}
        if "maven-source-plugin" not in ids or "maven-javadoc-plugin" not in ids: errors.append(f"{path} must attach source and Javadoc JARs")
    ...
    if "raw.githubusercontent.com/carstenartur/jgit-storage-hibernate/maven-repository" not in url: errors.append("consumer default repository URL is wrong")
    artifacts={txt(d,"m:artifactId") for d in consumer.findall("m:dependencies/m:dependency",NS)}
    if artifacts!=JARS: errors.append(f"consumer artifacts mismatch: {sorted(artifacts)}")
    if Path('.github/workflows/temp-public-repo-source-export.yml').exists(): errors.append("temporary source export workflow must be removed")
    if errors:
