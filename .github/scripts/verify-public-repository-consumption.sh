...
resolve_once() {
  ...
  if grep -Eiq 'maven\.pkg\.github\.com|central\.sonatype\.com|repo1\.maven\.org/.*/jgit-storage-hibernate' "$LOG"; then
    echo "Project artifacts were resolved from an unintended publishing service" >&2; return 1
  fi
  local base="$LOCAL_REPO/io/github/carstenartur" artifact extension expected
  for artifact in jgit-storage-hibernate-parent jgit-storage-hibernate-core jgit-storage-hibernate-search jgit-storage-hibernate-java-analysis jgit-storage-hibernate-architecture jgit-storage-hibernate-benchmarks; do
  ...
  echo "Anonymous public repository consumption verified for $VERSION from $REPOSITORY_URL"
}
...
echo "Could not resolve all $VERSION artifacts anonymously from $REPOSITORY_URL" >&2
exit 1
