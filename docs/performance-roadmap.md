...
2. Replace identity-generated chunk IDs with a key strategy that permits real JDBC insert batching.
3. Measure 16, 32 and 64 statement batch sizes before selecting a default.
4. Implement chunk read-ahead and compare one-, four- and sixteen-chunk windows.
5. Add `ReceivePack` initial/incremental push and `UploadPack` clone/fetch benchmarks.
6. Record SQL statement counts, transaction counts, connection acquisitions and repository-lock wait time per workload.
